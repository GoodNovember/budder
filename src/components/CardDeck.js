const { CardItem } = require("./CardItem.js")
	...
	render(){
		...
		return (
			<div className="card-deck">
				<div className="card-deck__top-bar">
				{ 
					(disabledCardCount > 0) ? ( 
						<div onClick={this.toggleEnabledOnlyMode}>
							{isEnabledOnlyMode?'Show Disabled':'Hide Disabled'}
						</div>
					) : null
				}
				</div>
				<div className="card-deck__card-list">
				{ CardCollection }
				</div>
				<div className="card-deck__bottom-bar">
					<div className="card-deck__button-corral">
						{ 
							(canShowCard && !selectedIsShown) ? (
								<div className="card-deck__control-button --show" onClick={ handleShowCardClick }>
						...
						<div onClick={handleBlackoutButtonClick} className={`card-deck__control-button --blackout ${isBlackoutMode?'--on':'--off'}`}>
							<i className="fa fa-crow"></i>
						</div>

						<div onClick={handlePreviousClick} className={`card-deck__control-button --previous ${canGoBack?'--enabled':'--disabled'}`}>
							<i className="fa fa-step-backward"></i>
						</div>
						
						<div onClick={handlePlayButtonClick} className={`card-deck__control-button --play ${isPlayEnabled?'--enabled':'--disabled'}`}>
							<i className="fa fa-play"></i>
						</div>
						
						<div onClick={handleNextClick} className={`card-deck__control-button --next ${canGoForward?'--enabled':'--disabled'}`}>
