export class EditableText extends React.Component{
	...
	render(){
		...
		return (
			<div className={`${className?className:''} editable-text ${isEditMode?'editable-text--edit-mode':''}`}>
				<form className="editable-text__form" onSubmit={this.handleSubmit}>
					<input className="editable-text__input" onChange={handleChange} ref={textElm} onBlur={handleBlur} type="text" value={this.state.value} placeholder={hintText}/>
				</form>
				<div className="editable-text__label" onClick={handleClick}>{value||hintText}</div>
			</div>
		)
